...
export default function Nav() {
  ...
  return (
    <div className="w-full p-4 text-white flex justify-between items-center xl:px-16">
      ...
      <div className="lg:hidden flex cursor-pointer " onClick={toggleMobileMenu}>
        {isMobileMenuOpen ? <FaTimes size={30} className="text-white" /> : <FaBars size={30} className="text-black"/>}
      </div>
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="absolute top-0 left-0 w-full h-screen bg-black text-white flex flex-col items-center justify-center space-y-6">
          <Link href="/products" onClick={toggleMobileMenu}>Medical Products</Link>
          <Link href="/retail" onClick={toggleMobileMenu}>Retail</Link>
          <Link href="/consulting" onClick={toggleMobileMenu}>Consulting</Link>
          <button
